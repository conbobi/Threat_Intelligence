...
from services.news_service import get_news_list, create_news
...
@admin_required
def news_list():
    news_list = get_news_list(only_visible=False)  # admin xem cả bài ẩn
    return render_template('admin/news_list.html', news_list=news_list, username=session.get('admin_username'))

@admin_bp.route('/news/create', methods=['GET', 'POST'])
@admin_required
def news_create():
    if request.method == 'GET':
        return render_template('admin/news_create.html', username=session.get('admin_username'))
    ...
    status = request.form.get('trang_thai', 'HIEN').strip()
